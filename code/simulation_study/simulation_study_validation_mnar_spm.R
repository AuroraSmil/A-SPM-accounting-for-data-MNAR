...
spm_sim <-readRDS(file = "/home/aurorach/data_HUNT_aurora/master/spm_inla_sim_HUNT23.RData")
...
dataH34[, age_effect_spm := predict.gam(age_func_spm, data.frame(age_round = age_round)) ]
dataH34[, intercept := rep(b_0, n) + age_effect_spm]
...
dir <- glue("results/simulations/{today}")
write.csv(mu_given_vec, file = glue("{dir}/{n_sim}_mu_given_vec"), row.names = FALSE)
write.csv(mu_vec, file = glue("{dir}/{n_sim}_mu_vec"), row.names = FALSE)
